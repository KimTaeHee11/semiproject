...
export const apiCreatePost = async (data) => {
    ...
    const response = await axios.post(`${baseUrl}/${what}Posts`, data, {
        headers: {
...
export const apiFetchPostList = async (page = 1, size = 3, query = undefined) => {
    ...
    //alert(page);
    const response = await axiosInstance.get(`/${what}Posts`, { params: { page, size, query } });
    // `/posts?page=${page}&size=${size}&query=${query}`
    return response.data;
};
//---post 글 삭제하기 ----------------------------------
export const apiDeletePost = async (id) => {
    const response = await axiosInstance.delete(`/${what}Posts/${id}`);
    return response.data;
};
//---post 단건 가져오기 -----------------------------
export const apiFetchPostById = async (id) => {
    const response = await axiosInstance.get(`/${what}Posts/${id}`);
...
export const apiUpdatePost = async (id, formData) => {
    await axiosInstance.put(`/${what}Posts/${id}`, formData, {
        headers: {
