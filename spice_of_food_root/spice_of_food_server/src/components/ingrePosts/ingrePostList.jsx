...
export default function IngrePostList() {
    const postList = useIngrePostStore((s) => s.postList);
    const totalCount = useIngrePostStore((s) => s.totalCount);
    const fetchPostList = useIngrePostStore((s) => s.fetchPostList);
    const totalPages = useIngrePostStore((s) => s.totalPages);
    const setPage = useIngrePostStore((s) => s.setPage);
    const page = useIngrePostStore((s) => s.page);
                ...
                postList.map((post, index) => (
                    ...
                    >
                        <div style={{ width: '30%' }} className='text-center'>
                            {post.file ? (
                                <img
                                    src={`http://localhost:7777/uploads/ingrePost/${post.file}`}
                                    alt={post.file}
                                    style={{ width: '90%' }}
                        ...
                        <div style={{ width: '70%' }} className='p-3'>
                            ...
                            <Link to={`/ingrePosts/ingrePostView/${post.id}`} style={{ textDecoration: 'none' }}>
                                <h3>{post.title}</h3>
                            </Link>
