...
export default function RecipePostView() {
    const { id } = useParams();
    const navigate = useNavigate();

    const authUser = useAuthStore((s) => s.authUser);

    const deletePost = useRecipePostStore((s) => s.deletePost);

    const post = useRecipePostStore((s) => s.post);
    const fetchPostById = useRecipePostStore((s) => s.fetchPostById);
    const postErr = useRecipePostStore((s) => s.postErr);
    const resetPostErr = useRecipePostStore((s) => s.resetPostErr);
            ...
            <div className='row my-3'>
                <div className='col-md-10 offset-md-1 px-3'>
                    ...
                    <div className='text-end my-2'>
                        <Link to={`/recipePosts/recipePostEdit/${id}`} onClick={go}>
                    ...
                    <div className='card'>
                        <div className='card-body'>
                            ...
                            <div style={{ marginBottom: '1rem' }} className='text-center'>
                                <img
                                    src={`http://localhost:7777/uploads/recipePost/${post.file ?? 'noimage.jpg'}`}
                                    alt={`${post.file ?? 'noimage'}`}
                                    style={{ maxWidth: '100%', borderRadius: '0.5rem' }}
                                />
                            </div>
                            <div>
                                <h6>{post.recipe_name}</h6>
