...
exports.createPost = async (req, res) => {
    const { name, title, recipe_name, ingredients, content } = req.body;
    const file = req.file;
    ...
    if (!name || !title || !ingredients) {
        return res.status(400).json({ result: 'fail', message: '필수 항목을 입력해야 해요' });
    }
    try {
        const sql = `insert into recipePosts(name,title,recipe_name,ingredients,content,attach)
                    values(?,?,?,?,?,?)`;

        const [result] = await pool.query(sql, [name, title, content, recipe_name, ingredients, fileName]);
...
exports.listPost = async (req, res) => {
    ...
    try {
        //1. 전체 게시글 수 가져오기
        const sql = `select count(id) count from recipePosts`;
        ...
        //2. 전체 게시목록 가져오기
        const sql2 = `select id,name,title,recipe_name,ingredients,content,
        attach as file, date_format(wdate,'%Y-%m-%d') as wdate
        from recipePosts order by id desc limit ? offset ?`; //limit ?,? ==> [offset, size]
...
exports.getPostById = async (req, res) => {
    ...
    console.log('id: ', id);
    try {
        const sql = `select id,title,recipe_name,ingredients,content,name, attach as file,
            date_format(wdate,'%Y-%m-%d %H:%i:%s') wdate
            from recipePosts where id=?`;
...
exports.deletePost = async (req, res) => {
    ...
    try {
        ...
        const sql = `select attach as file from recipePosts where id=?`;
        ...
        let filePath = null;
        if (data.file) {
            filePath = path.join(__dirname, '..', '..', 'public', 'uploads', 'recipePost', data.file);
        }
        console.log('filePath: ', filePath);

        //2. DB에서 해당 글 삭제 처리
        const sql2 = `delete from recipePosts where id=?`;
...
exports.updatePost = async (req, res) => {
    ...
    try {
        ...
        let sql2 = `update recipePosts set name=?,recipe_name=?,ingredients=?,title=?,content=? `;

        const params = [name, title, recipe_name, ingredients, content];
...
// 재료 데이터로 부터 관련된 레시피 정보 검색
// 재료로부터 재료 카테고리 검색
// 레시피에 대해 세부 재료 카테고리 seach -> 재료 카테고리 search -> join 실행
